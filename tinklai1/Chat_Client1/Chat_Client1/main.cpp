...
int main()
{
	WSAData wsa_data; //The WSADATA structure contains information about the Windows Sockets implementation.
	...
	// Initialize Winsock
	iResult = WSAStartup(MAKEWORD(2, 2), &wsa_data); //(Low, bHigh), function initiates use of the Winsock DLL by a process.
	if (iResult != 0) {
	...
	hints.ai_family = AF_UNSPEC;		// (IPv4 vs. IPv6)
	hints.ai_socktype = SOCK_STREAM;   //(datagram vs. straming)
	hints.ai_protocol = IPPROTO_TCP;	// (TCP vs. UDP) , null=dont care
